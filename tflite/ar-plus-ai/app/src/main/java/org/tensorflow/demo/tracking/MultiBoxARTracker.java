 ...
 */
public class MultiBoxARTracker {
  ...
  private Scene scene;
  private Uri model = Uri.parse("andy.sfb");

  public MultiBoxARTracker(final Context context, final ArFragment arFragment) {
  ...
  public synchronized void onFrame(
    ...
    // hopefully by now the AR scene is initialized
  ...
  private void handleDetection(
    ...
    // only use this to update items on the UI
    arFragment.getActivity().runOnUiThread(() -> {
      RectF object = potential.second.getLocation();
      Frame frame = arFragment.getArSceneView().getArFrame();
      List<HitResult> hits;
      if (frame != null) {
        hits = frame.hitTest(object.centerX(), object.centerY());
  ...
  private void addNodeToScene(ArFragment fragment, Anchor anchor, Renderable renderable) {
    logger.i("*** Adding a node to the scene");
    ...
    node.select();
    if (objectTracker == null) {
      // This is a stupid hack as to how to prevent object overlap without Object Detection
      if (scene.overlapTest(node) != null) {
